...
func OnDealSectorPreCommitted(ctx context.Context, api getCurrentDealInfoAPI, eventsApi sectorCommittedEventsAPI, provider address.Address, proposal market.DealProposal, publishCid *cid.Cid, callback storagemarket.DealSectorPreCommittedCallback) error {
	...
	checkFunc := func(ts *types.TipSet) (done bool, more bool, err error) {
		di, isActive, publishTs, err := checkIfDealAlreadyActive(ctx, api, ts, proposal, publishCid)
		if err != nil {
	...
	called := func(msg *types.Message, rec *types.MessageReceipt, ts *types.TipSet, curH abi.ChainEpoch) (more bool, err error) {
		...
		dealID, _, _, err := GetCurrentDealInfo(ctx, ts, api, proposal, publishCid)
		if err != nil {
			return false, err
		}
...
func OnDealSectorCommitted(ctx context.Context, api getCurrentDealInfoAPI, eventsApi sectorCommittedEventsAPI, provider address.Address, sectorNumber abi.SectorNumber, proposal market.DealProposal, publishCid *cid.Cid, callback storagemarket.DealSectorCommittedCallback) error {
	...
	// First check if the deal is already active, and if so, bail out
	checkFunc := func(ts *types.TipSet) (done bool, more bool, err error) {
		_, isActive, _, err := checkIfDealAlreadyActive(ctx, api, ts, proposal, publishCid)
		if err != nil {
			// Note: the error returned from here will end up being returned
	...
	called := func(msg *types.Message, rec *types.MessageReceipt, ts *types.TipSet, curH abi.ChainEpoch) (more bool, err error) {
		...
		// Get the deal info
		dealID, sd, _, err := GetCurrentDealInfo(ctx, ts, api, proposal, publishCid)
		if err != nil {
...
func checkIfDealAlreadyActive(ctx context.Context, api getCurrentDealInfoAPI, ts *types.TipSet, proposal market.DealProposal, publishCid *cid.Cid) (abi.DealID, bool, types.TipSetKey, error) {
	di, sd, publishTs, err := GetCurrentDealInfo(ctx, ts, api, proposal, publishCid)
	if err != nil {
	...
	if sd.State.SlashEpoch > 0 {
		return 0, false, types.EmptyTSK, xerrors.Errorf("deal %d was slashed at epoch %d", di, sd.State.SlashEpoch)
	}
